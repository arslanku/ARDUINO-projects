...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QMessageBox, QWidget, \
    QPushButton, QComboBox, QLabel, QTextEdit, QHBoxLayout, QCheckBox, QGridLayout, QLineEdit

baudRate = 9600  # НЕ ЗАБУДЬ УСТАНОВИТЬ СКОРОСТЬ ПОРТА !
...
class MainWindow(QMainWindow):
    ...
    def send_data(self):
        if self.serial and self.serial.isOpen():
            ...
            self.x_input.clear()
            self.y_input.clear()
            self.log_text_edit.append(f"[DATA] Отправлены данные    X:{x}  Y:{y}\n")
    ...
    def read_from_serial(self):
        ...
        if self.serial is not None and self.serial.isOpen():
            if self.serial.canReadLine():
                ...
                if data:
                    text = data.data().decode('utf-8', errors='ignore').strip()  # или использовать это
