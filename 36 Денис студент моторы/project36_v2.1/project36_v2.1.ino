...
int sensor1 = 0; // переменная, для храния значения с датчика 1
int sensor2 = 0; // переменная, для храния значения с датчика 2
int sensor3 = 0; // переменная, для храния значения с датчика 3
int sensor4 = 0; // переменная, для храния значения с датчика 4
...
int eeprom_address;                   // переменная для адреса в EEPROM, по которому будем записывать данные с датчиков
...
void loop()
{
  ...
  if (!motor1_running && !motor2_running)
  {
    ...
    EEPROM.get(1010, eeprom_address);
    delay(100);
    ...
    {
      eeprom_address = 0;
      Serial.println("[ВНИМАНИЕ] Память заполнена. Адрес был сброшен до 0");
    }
    // записываем новый адрес в EEPROM
    EEPROM.put(1010, eeprom_address);
    delay(100);
